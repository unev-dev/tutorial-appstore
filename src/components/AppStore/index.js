...
class AppStore extends Component {
  state = {
    selekt: tabsList[0].tabId,
    si: '',
  }

  showApps = () => {
    const {selekt, si} = this.state
    const sil = si.toLowerCase()
    const listedApps = appsList.filter(each => each.category === selekt)
    const listedAppsone = listedApps.filter(each =>
      each.appName.toLowerCase().includes(sil),
    )
    console.log(listedAppsone)
    return listedAppsone
  }

  someprogTochange = tabId => {
    this.setState({selekt: tabId})
  }

  somethingShould = event => {
    const xx = event.target.value
    this.setState({si: xx})
  }

  render() {
    const al = this.showApps()
    const {selekt} = this.state
    return (
      <div className="bg">
        <h1>App Store</h1>
        <div className="se-c">
        ...
        <ul className="btn-c">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              tabdetails={each}
              someprogTochange={this.someprogTochange}
              selbtn={each.tabId === selekt}
            />
          ))}
        </ul>
        <ul className="app-c">
          {al.map(each => (
            <AppItem key={each.appId} appd={each} />
          ))}
        </ul>
      </div>
    )
